...
# Each entry: (background, foreground)  — tuples for immutability
...
_PALETTE_LEN = len(CATEGORY_COLORS_LIGHT)
...
def get_color(category: str, categories: list, dark_mode: bool,
    ...
    try:
        idx = categories.index(category) % _PALETTE_LEN
    except ValueError:
        idx = _PALETTE_LEN - 1
    return palette[idx]
...
def load_category_colors(config: dict) -> dict:
    ...
    for name, entry in raw.items():
        parsed = {}
        if "light" in entry:
            parsed["light"] = tuple(entry["light"])
        if "dark" in entry:
            parsed["dark"] = tuple(entry["dark"])
        if parsed:
            result[name] = parsed
    return result
